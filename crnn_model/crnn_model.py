...
class ShadowNet(cnn_basenet.CNNBaseModel):
    ...
    def sequence_length(self):
        """
        sequence length,这个得根据实现卷积和池化的参数来确定
        :return:
        """
    ...
    def _conv_stage(self, inputdata, out_dims, name=None):
        ...
        conv = self.conv2d(inputdata=inputdata, out_channel=out_dims,
                           kernel_size=3, stride=1, use_bias=False,padding='SAME',
                           name=name)
        relu = self.relu(inputdata=conv)
        
        #kernel=[1,2,2,1],pooling output [batch,h/2,w/2,out_channel]
        max_pool = self.maxpooling(inputdata=relu, kernel_size=2, stride=2,padding='VALID')
        return max_pool
    ...
    def _feature_sequence_extraction(self, inputdata):
        ...
        """
        tensor_dict = dict()
        conv1 = self._conv_stage(inputdata=inputdata, out_dims=64, name='conv1')  #eg. batch*16*50*64
        tensor_dict['conv1'] = conv1

        conv2 = self._conv_stage(inputdata=conv1, out_dims=128, name='conv2')  #eg. batch*8*25*128
        ...
        max_pool6 = self.maxpooling(inputdata=relu6,
                                    kernel_size=[2, 1], stride=[2, 1],padding='VALID')  # batch*2*25*512
        ...
        conv7 = self.conv2d(inputdata=max_pool6, out_channel=512,
                            kernel_size=2, stride=[2, 1], use_bias=False,padding='SAME',
    ...
    def _map_to_sequence(self, inputdata):
        ...
        :return: a NHWC tensor
        Map-to-Sequence表示成特征向量
        CNN最后一层的feature map上的每个滑动窗口直接拉成一列一列的特征输出到RNN(LSTM)
        """
        shape = inputdata.get_shape().as_list()
        assert shape[1] == 1  # H of the feature map must equal to 1
        #把axis==1上，维度为1的删掉
        return self.squeeze(inputdata=inputdata, axis=1) # batch*25*512
    ...
    def _sequence_label(self, inputdata):
        ...
        if self._rnn_cell_type == 'lstm':
            with tf.variable_scope('LSTMLayers'):
                ...
                #batch*32*100过来的图，到这里会变为 batch*25*512
    ...
    def build_shadownet(self, inputdata):
        ...
        with tf.variable_scope('cnn_subnetwork'):
            # first apply the cnn feature extraction stage,特征提取层
            cnn_out, tensor_dict = self._feature_sequence_extraction(inputdata=inputdata)

            # second apply the map to sequence stage，表示成特征向量,卷积层和循环层之间的桥梁
            sequence = self._map_to_sequence(inputdata=cnn_out)

            # third apply the sequence label stage,序列标定层,batch*32*100的到这会变成sequence batch*25*512，然后去进行转录
