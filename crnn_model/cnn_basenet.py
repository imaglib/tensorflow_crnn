...
class CNNBaseModel(metaclass=ABCMeta):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME', stride=1, w_init=None, b_init=None,
        ...
        如本例中就是 
        out_height=ceil(32/1)=32
        out_width =ceil(config.cfg.TRAIN.IMAGE_WIDTH/1)=config.cfg.TRAIN.IMAGE_WIDTH
    ...
    def maxpooling(inputdata, kernel_size, stride=None, padding='VALID', data_format='NHWC', name=None):
        ...
        #kernel=[1,kernel_size,kernel_size,1],pooling output [batch,h/kernel_size,w/kernel_size,out_channel]
    ...
    def layerbn(inputdata, is_training, name):
        """
        bn Batch Normalization，对数据进行归一化，去除异常数据，加速收敛
        :param inputdata:
        :param is_training:
        :param name:
        :return:
    ...
    # 自己实现的batch normalization
    @staticmethod
    def layerbn_v2(inputdata, phase, shift=True, scale=True, momentum=0.99,
                   eps=1e-3, internal_update=False, scope=None, reuse=None):

        def _assign_moving_average(_orig_val, _new_val, _momentum, name):
            with tf.name_scope(name):
                _scaled_diff = (1 - _momentum) * (_new_val - _orig_val)
                return tf.assign_add(_orig_val, _scaled_diff)

        c = inputdata.get_shape_as_list()[-1]
        ndim = len(inputdata.shape)
        var_shape = [1] * (ndim - 1) + [c]

        with tf.variable_scope(scope, 'batch_norm', reuse=reuse):
            def training():
                m, v = tf.nn.moments(inputdata, range(ndim - 1), keep_dims=True)
                ...
                if internal_update:
                    with tf.control_dependencies([update_m, update_v]):
                        _output = (inputdata - m) * tf.rsqrt(v + eps)
                else:
                    _output = (inputdata - m) * tf.rsqrt(v + eps)
                return _output
